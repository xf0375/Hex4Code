...
import { routeTask, detectConfiguredProviders } from "@hex4/core/models/model-router";
...
export function App({ projectRoot, version = "", onRestart }: AppProps): React.ReactElement {
  ...
  const sessionManager = useMemo(() => {
    ...
    const { detectProjectMode, getEffectiveMode, getModeLabel } = require("@hex4/core/agent-mode");
    const envOverride = process.env.HEX4CODE_AGENT_MODE;
    const validatedOverride: "hex4" | "general" | undefined =
      envOverride === "hex4" || envOverride === "general" ? envOverride : undefined;
    const effectiveMode = getEffectiveMode(
      { mode: detectProjectMode(projectRoot), autoDetect: !validatedOverride, override: validatedOverride },
      projectRoot
    );
    // Set global mode flag for executor
    const { setAgentMode, getAgentMode } = require("@hex4/core/agent-mode");
    setAgentMode(effectiveMode);
    process.stderr.write(`[HEX4] 项目模式: ${getModeLabel(effectiveMode)}${validatedOverride ? " (env override)" : ""}\n`);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      try {
        const { detectConfiguredProviders, getUnconfiguredProviders } = require("@hex4/core/models/model-router");
        ...
        if (configured.length === 0 && available.length > 0) {
          ...
          process.stderr.write(chalk.dim(`🔌 ${configured.length} provider(s) configured: ${configured.join(", ")}\n`));
        }
      } catch { /* background check */ }
    }, 2000);
    ...
    async (submission: PromptSubmission) => {
      ...
      if (submission.command === "hex4") {
        ...
        if (subCmd === "new" || subCmd === "n") {
          ...
          process.stdout.write(chalk.dim("  Use templates in ~/.agents/skills/hex4-*/\n"));
          process.stdout.write("\n");
          process.stdout.write("Available templates:\n");
          process.stdout.write("  /hex4 new module     - C module (hex4_*.h + hex4_*.c + test_*.c)\n");
          process.stdout.write("  /hex4 new crypto     - Crypto algorithm (in HEX4\u5bc6\u7801/)\n");
          process.stdout.write("  /hex4 new test       - Test file using T()/OK()/NG()\n");
          process.stdout.write("  /hex4 new alu        - Ternary ALU core (in HEX4-Lite-IP\u6838/)\n");
        } else if (subCmd === "build") {
          process.stdout.write(chalk.cyan("Tip: use the `build` tool with project=\"<subdir>\"\n"));
        } else if (subCmd === "test") {
          process.stdout.write(chalk.cyan("Tip: use the `test` tool with project=\"<subdir>\"\n"));
        } else {
          process.stdout.write(chalk.yellow("Commands: /hex4 new [module|crypto|test|alu]\n"));
        }
        process.stdout.write("\n");
        return;
      }
      if (submission.command === "sessions") {
        ...
        if (subCmd === "list" || subCmd === "ls") {
          ...
          for (const s of all) {
            ...
            process.stdout.write(`  ${chalk.green(s.id.substring(0, 8))}  ${chalk.dim(date.toLocaleString())}  "${label}"\n`);
          }
          process.stdout.write(chalk.dim("─────────────────\n"));
        } else if (subCmd === "delete" || subCmd === "rm") {
          if (!arg) { process.stdout.write(chalk.red("Usage: /sessions delete <session-id>\n")); }
          else {
            const found = sessionManager.deleteSession?.(arg);
            process.stdout.write(found ? chalk.green(`✅ Session "${arg.substring(0, 8)}" deleted.\n`) : chalk.red(`❌ Session "${arg.substring(0, 8)}" not found.\n`));
          }
        } else if (subCmd === "export") {
          if (!arg) { process.stdout.write(chalk.red("Usage: /sessions export <session-id>\n")); }
          else {
            const messages = sessionManager.listSessionMessages(arg);
            const outPath = path.join(projectRoot, `session-${arg.substring(0, 8)}.md`);
            const md = messages.map(m => `## ${m.role}\n\n${typeof m.content === 'string' ? m.content : JSON.stringify(m.content, null, 2)}`).join("\n\n");
            fs.writeFileSync(outPath, md, "utf8");
            process.stdout.write(chalk.green(`✅ Session exported to ${outPath}\n`));
          }
        } else {
          process.stdout.write(chalk.yellow("Commands: list, delete <id>, export <id>\n"));
        }
        process.stdout.write("\n");
        return;
      }
      ...
      if (submission.command === "model") {
        ...
        if (subCmd === "list" || subCmd === "ls") {
          ...
          for (const p of PROVIDERS) {
            ...
            process.stdout.write(`  ${icon} ${chalk.bold(p.name)}  ${isConfigured ? chalk.green("configured") : chalk.dim("unconfigured")}\n`);
            for (const m of p.models) {
              const caps = Array.isArray(m.capabilities) ? m.capabilities.join(", ") : "";
              const cost = m.costPer1MInput ? ` $${m.costPer1MInput}/${m.costPer1MOutput} per 1M tok` : "";
              const ctx = m.contextWindow ? chalk.dim(` (${Math.round(m.contextWindow / 1000)}K ctx`) : "";
              process.stdout.write(`    ${chalk.cyan(m.id)}${ctx}${cost ? chalk.dim(cost) : ""}${chalk.dim(")")}\n`);
      ...
      if (submission.command === "cost") {
        ...
        try {
          ...
          if (!fs.existsSync(sessionsPath)) {
            ...
            let totalCost = 0, totalTokens = 0, sessionCount = 0;
            for (const e of entries) {
              const cost = typeof e.totalCost === "number" ? e.totalCost : 0;
              const tokens = typeof e.activeTokens === "number" ? e.activeTokens : 0;
              if (cost > 0 || tokens > 0) {
                totalCost += cost; totalTokens += tokens; sessionCount++;
                process.stdout.write(`  ${chalk.cyan(e.id?.substring(0, 8) || "?")}  ${(e.summary || "(no summary)").substring(0, 40)}\n`);
                process.stdout.write(chalk.dim(`      Cost: $${cost.toFixed(6)}  Tokens: ${tokens.toLocaleString()}\n`));
              }
            }
            process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
            process.stdout.write(`  Sessions: ${sessionCount}\n`);
            process.stdout.write(`  Total Cost: ${chalk.green(`$${totalCost.toFixed(6)}`)}\n`);
            process.stdout.write(`  Total Tokens: ${totalTokens.toLocaleString()}\n`);
            if (totalCost > 0) {
              process.stdout.write(`  Avg/Session: $${(totalCost / Math.max(sessionCount, 1)).toFixed(6)}\n`);
            }
          }
        } catch (e: unknown) {
          process.stdout.write(chalk.red(`  Error: ${e instanceof Error ? e.message.slice(0, 100) : String(e)}\n`));
        }
        process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        process.stdout.write("\n");
        return;
      ...
      if (submission.command === "recommend") {
        const { getSmartRecommendation, detectConfiguredProviders } = await import("@hex4/core/models/model-router");
        const configured = detectConfiguredProviders(process.env);
        process.stdout.write(chalk.bold.cyan("\nSmart Model Recommendation\n"));
        process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        process.stdout.write(`Configured: ${configured.join(", ") || chalk.yellow("none")}\n`);
        const tasks = [
        ...
        for (const task of tasks) {
          const recs = getSmartRecommendation(task.id as any, configured, "balanced");
          process.stdout.write(`\n▸ ${chalk.bold(task.label)}\n`);
          if (recs.length === 0) { process.stdout.write("   No suitable models\n"); }
          else {
            recs.forEach((r, i) => {
              const icon = i === 0 ? "★" : " ";
      ...
      if (submission.command === "vote") {
        const text = (submission.text ?? "").trim();
        const { parallelVote, detectConfiguredProviders } = await import("@hex4/core/models/model-router");
        const configured = detectConfiguredProviders(process.env);
        if (configured.length < 2) {
          process.stdout.write(chalk.yellow("⚠️  Need at least 2 configured providers for voting.\n"));
          return;
        }
        const strategy = text.includes("consensus") ? "consensus" as const : text.includes("fastest") ? "fastest" as const : "majority" as const;
        const promptText = text.replace(/^(majority|consensus|fastest)\s*/i, "").trim();
        if (!promptText) {
          process.stdout.write(chalk.yellow("Usage: /vote [majority|consensus|fastest] <prompt>\n"));
          return;
        }
        process.stdout.write(chalk.bold.cyan(`\nVoting (${strategy}) — ${configured.length} providers...\n`));
        const result = await parallelVote(promptText, configured, { strategy, modelCount: 3 });
        process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        for (const r of result.responses) {
          const icon = r.error ? "❌" : "✅";
          process.stdout.write(`${icon} ${chalk.bold(r.label)} (${r.provider}) — ${r.latencyMs}ms\n`);
          if (r.error) process.stdout.write(chalk.red(`   ${r.error.slice(0, 100)}\n`));
          else process.stdout.write(chalk.dim(`   ${r.response.slice(0, 200)}...\n`));
        }
        process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        if (result.success) {
      ...
      if (submission.command === "cache") {
        try {
          ...
          process.stdout.write(`Models: ${stats.totalModels.join(", ") || "none"}\n`);
          process.stdout.write(`Hits/Misses: ${stats.hits}/${stats.misses}\n`);
          process.stdout.write(`Hit Rate: ${(stats.hitRate * 100).toFixed(1)}%\n`);
          process.stdout.write(chalk.dim("───────────────────────────────────────\n\n"));
        } catch (e: unknown) {
          process.stdout.write(chalk.red(`Error: ${e instanceof Error ? e.message : String(e)}\n`));
        }
        return;
      }
      ...
      if (submission.command === "benchmark") {
        const text = (submission.text ?? "").trim();
        if (!text) { process.stdout.write(chalk.yellow("Usage: /benchmark <prompt>\n")); return; }
        const { benchmarkModels, detectConfiguredProviders } = await import("@hex4/core/models/model-router");
        const configured = detectConfiguredProviders(process.env);
        ...
        for (const r of result.results) {
          const icon = r.error ? "❌" : "✅";
          process.stdout.write(`${icon} ${chalk.bold(r.label)} (${r.provider})\n`);
          process.stdout.write(chalk.dim(`   ${r.latencyMs}ms · ${r.responseLength} chars · $${r.cost.toFixed(6)}\n`));
          if (r.error) process.stdout.write(chalk.red(`   ${r.error.slice(0, 100)}\n`));
        }
        process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        process.stdout.write(chalk.green(`⚡ Fastest: ${result.fastest} · 💰 Cheapest: ${result.cheapest} · 📝 Longest: ${result.longest}\n\n`));
        return;
      }
      ...
      if (submission.command === "quota") {
        const text = (submission.text ?? "").trim();
        const { loadQuota, setQuotaLimit, checkQuota, saveQuota } = await import("@hex4/core/models/model-router");
        const parts = text.split(/\s+/).filter(Boolean);
        ...
        if (subCmd === "set" && parts.length >= 3) {
          ...
          process.stdout.write(chalk.green(`Quota set: ${tokenLimit.toLocaleString()} tokens / $${costLimit.toFixed(2)} per month\n`));
          ...
          process.stdout.write(chalk.bold.cyan("\nQuota Status\n"));
          process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
          process.stdout.write(`Token Limit: ${q.monthlyTokenLimit > 0 ? q.monthlyTokenLimit.toLocaleString() : "unlimited"}\n`);
          process.stdout.write(`Cost Limit: ${q.monthlyCostLimit > 0 ? `$${q.monthlyCostLimit.toFixed(2)}` : "unlimited"}\n`);
          process.stdout.write(`Used Tokens: ${q.currentTokens.toLocaleString()}\n`);
          process.stdout.write(`Used Cost: $${q.currentCost.toFixed(6)}\n`);
          process.stdout.write(`Usage: ${status.usagePercent.toFixed(1)}%\n`);
          process.stdout.write(`Status: ${status.allowed ? chalk.green("OK") : chalk.red("EXCEEDED")}\n`);
          process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
          process.stdout.write(chalk.dim("  /quota set <tokenLimit> <costLimit>  — set limits\n"));
          process.stdout.write(chalk.dim("  /quota reset                         — reset monthly usage\n\n"));
        }
        return;
      }
      ...
      if (submission.command === "insights") {
        const { getRouteInsights, getSuggestedWeights } = await import("@hex4/core/models/model-router");
        const insights = getRouteInsights();
        process.stdout.write(chalk.bold.cyan("\nRoute Insights\n"));
        process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        if (insights.length === 0) {
          process.stdout.write(chalk.yellow("No route history yet. Use the assistant to generate data.\n"));
        } else {
          for (const i of insights) {
            process.stdout.write(`\n${chalk.bold(i.taskType)} — ${i.totalCalls} calls\n`);
            process.stdout.write(`  Success: ${(i.successRate * 100).toFixed(0)}%  Latency: ${i.avgLatency.toFixed(0)}ms  Cost: $${i.avgCost.toFixed(6)}\n`);
            process.stdout.write(`  Best: ${chalk.green(i.bestModel)}\n`);
            const weights = getSuggestedWeights(i.taskType);
            if (weights) process.stdout.write(chalk.dim(`  Suggested weights: cost=${weights.cost} cap=${weights.capability} ctx=${weights.context} speed=${weights.speed}\n`));
            for (const m of i.modelRankings.slice(0, 3)) {
              process.stdout.write(chalk.dim(`    ${m.model}: ${(m.successRate * 100).toFixed(0)}% success · ${m.avgLatency.toFixed(0)}ms avg · ${m.calls} calls\n`));
            }
          }
        }
        process.stdout.write(chalk.dim("───────────────────────────────────────\n\n"));
      ...
      if (submission.command === "compact") {
        ...
        process.stdout.write(chalk.green("Compaction is handled automatically when context exceeds threshold.\n"));
        return;
      }
      ...
      if (submission.command === "provider") {
        ...
        if (subCmd === "list" || subCmd === "ls") {
          ...
          for (const p of PROVIDERS) {
            ...
            process.stdout.write(`  ${icon} ${chalk.bold(p.name)}  ${status}\n`);
            process.stdout.write(chalk.dim(`     ${p.defaultBaseURL}\n`));
            process.stdout.write(chalk.dim(`     ${p.models.length} models · ${p.models.map(m => m.label).join(", ")}\n`));
          }
          process.stdout.write(chalk.dim("─────────────────\n"));
        } else if (subCmd === "set") {
          ...
          if (!provider) {
            process.stdout.write(chalk.red(`❌ Provider "${providerName}" not found. Use /provider list\n`));
          } else {
            process.stdout.write(chalk.cyan(`\nEnter API key for ${provider.name}\n`));
            process.stdout.write(chalk.dim(`  (press Enter to use ENV ${provider.apiKeyEnv})\n`));
            process.stdout.write("> ");
            ...
            process.stdout.write(chalk.dim(`> ${key ? "************" : "(using ENV)"}\n`));

            if (key) {
              const settingsPath = path.join(os.homedir(), ".hex4", "settings.json");
              ...
              fs.mkdirSync(path.join(os.homedir(), ".hex4"), { recursive: true });
              fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2), "utf8");
              process.stdout.write(chalk.green(`✅ ${provider.name} API key saved to ~/.hex4/settings.json\n`));
              // ── API Key 验证 ──────────────────────────────
              process.stdout.write(chalk.dim(`⏳ Testing connection...`));
              try {
                const { testProviderConnection } = await require("@hex4/core/models/model-router");
                const testModel = provider.models.find(m => m.capabilities.includes("chat" as any)) || provider.models[0];
                const result = await testProviderConnection(testModel.id, key);
                if (result.ok) {
                  process.stdout.write("\r\x1B[K");
                  process.stdout.write(chalk.green(`✅ Connection OK (${result.latencyMs}ms)\n`));
                } else {
                  process.stdout.write("\r\x1B[K");
                  process.stdout.write(chalk.yellow(`⚠️  Connection failed: ${result.error.slice(0, 80)}\n`));
                }
              } catch { process.stdout.write("\r\x1B[K"); }
            } else {
              const envKey = process.env[provider.apiKeyEnv];
              if (envKey) {
                process.stdout.write(chalk.green(`✅ Using ENV ${provider.apiKeyEnv} for ${provider.name}\n`));
              } else {
                process.stdout.write(chalk.yellow(`⚠️  ENV ${provider.apiKeyEnv} not set\n`));
              }
            }
          }
          ...
          if (configured.length === 0) {
            ...
            process.stdout.write(chalk.dim("  Use /provider set <name> to configure one.\n"));
          } else {
            // 并行测试所有已配置的 Provider
            const results = await Promise.allSettled(
              configured.map(async (pid) => {
                const p = PROVIDERS.find(pr => pr.id === pid);
                if (!p) return { id: pid, ok: false, latency: 0, error: "Unknown provider" };
                const key = process.env[p.apiKeyEnv] || "";
                const testModel = p.models.find(m => m.capabilities.includes("chat" as any)) || p.models[0];
                const { testProviderConnection } = await require("@hex4/core/models/model-router");
                return { id: pid, result: await testProviderConnection(testModel.id, key, p.defaultBaseURL), provider: p };
              })
            );
            for (const r of results) {
              if (r.status === "fulfilled") {
                ...
                const latencyStr = d.result.ok ? chalk.dim(` (${d.result.latencyMs}ms)`) : "";
                const errorStr = d.result.ok ? "" : chalk.yellow(`  ${d.result.error.slice(0, 60)}`);
                const providerName = (d as any).provider?.name || d.id;
                process.stdout.write(`  ${icon} ${chalk.bold(providerName)}${latencyStr}\n`);
                if (errorStr) process.stdout.write(`     ${errorStr}\n`);
              } else {
                process.stdout.write(`  ${chalk.red("○")} ${chalk.bold(r.reason?.toString().slice(0, 40) || "unknown")}\n`);
              }
            }
          }
          process.stdout.write(chalk.dim("───────────────────────────────────────\n"));
        } else {
          process.stdout.write(chalk.yellow("Commands: list, set <name>, status, health\n"));
        }
        process.stdout.write("\n");
        return;
      }
...
export function readProjectSettings(projectRoot: string = process.cwd()): Hex4codeSettings | null {
...
export function writeProjectSettings(settings: Hex4codeSettings, projectRoot: string = process.cwd()): void {
  const settingsPath = getProjectSettingsPath(projectRoot);
  writeSettingsFile(settingsPath, settings);
}

function writeSettingsFile(settingsPath: string, settings: Hex4codeSettings): void {
...
export function resolveCurrentSettings(projectRoot: string = process.cwd()): ResolvedHex4codeSettings {
...
export function createOpenAIClient(projectRoot: string = process.cwd()): {
  ...
  const openaiClient = (client && "chat" in client)
