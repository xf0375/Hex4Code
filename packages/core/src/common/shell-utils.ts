...
export function findGitBashPath(): string {
  ...
  throw new Error(
    "Hex4Code on Windows requires Git Bash. Install Git for Windows, or ensure Git's bash.exe is available in PATH.",
  );
}

export function resolveWindowsGitBashPath(
  lookup: WindowsGitBashLookup,
): string | null {
  return firstExistingWindowsPath(
    [
      ...
      ...lookup
        .findExecutableCandidates("git")
        .flatMap(gitExecutableToBashCandidates),
    ],
    lookup.existsSync,
  );
}
