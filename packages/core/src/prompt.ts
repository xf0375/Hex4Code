...
export function getTools(
  ...
  tools.push({
    ...
    function: {
      name: "build",
      description:
        "Build a C/C++ project using Makefile or CMake. Returns structured compile errors/warnings (parsed from gcc/clang/fatal/make/linker output) instead of raw text. Prefer this over bash+make for building projects — it saves tokens by returning only structured error metadata rather than thousands of lines of raw compiler output.",
      parameters: {
        type: "object",
        properties: {
          project: {
  ...
  tools.push({
    ...
    function: {
      name: "codeIndex",
      description:
        "Search for C/C++ symbols (functions, structs, enums, macros, typedefs) across the codebase. Builds an in-memory index from .c/.h/.cpp files. Supports fuzzy matching. Primarily optimized for C; C++ class/namespace/template support is limited. Use this INSTEAD of grep/rg for finding definitions — it returns structured results with file:line and signatures, saving significant tokens compared to raw grep output.",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description:
              "Symbol name to search for. Case-insensitive fuzzy match. E.g., 'tc_propagate' finds hex4_tc_propagate, tc_propagator_run, etc.",
          },
          type: {
            type: "string",
            description:
              "Filter by symbol kind: 'function', 'struct', 'enum', 'macro', 'typedef'. Omit to search all kinds.",
          ...
          context: {
            type: "number",
            description: "Number of context lines to show (1-20, default 5).",
          },
        },
        required: ["query"],
        additionalProperties: false,
  ...
  tools.push({
    ...
    function: {
      name: "test",
      description:
        "Run a test binary and parse its output into structured PASS/FAIL results. Supports HEX4 (T()/OK()/NG() macros), GoogleTest ([  PASSED  ]/[  FAILED  ]), Catch2, CTest, TAP (ok/not ok), and common PASS/FAIL markers. NG() messages are best-effort parsed from nearby lines. Use this INSTEAD of bash+./test_all — it returns structured pass/fail counts and failure details rather than raw terminal output.",
      parameters: {
        type: "object",
        properties: {
          project: {
          ...
          binary: {
            type: "string",
            description:
              "Test binary filename or path. If omitted, auto-discovers from common names: test_all, test, run_tests, etc.",
          },
          filter: {
            type: "string",
            description:
          ...
          timeout: {
            type: "number",
            description: "Timeout in seconds (default 60, minimum 5).",
          },
        },
        required: [],
        additionalProperties: false,
  ...
  tools.push({
    ...
    function: {
      name: "git",
      description: "Git operations: status, diff, commit, log, review, pr.",
      parameters: {
        ...
        properties: {
          action: {
            ...
            description: "Action",
            enum: ["status", "diff", "commit", "log", "review", "pr"],
          },
          message: { type: "string" as const, description: "Commit message" },
          branch: { type: "string" as const, description: "Branch name" },
          target: { type: "string" as const, description: "Target branch" },
          staged: { type: "boolean" as const, description: "Staged only" },
        },
        required: ["action"],
      },
    },
